...
(test code-block
  (emit-identity "@begin[lang=lisp](code)test@end(code)"))

(test quotes
  (emit-identity "@q(test)")
  (emit-identity "@begin(quote)test@end(quote)"))

(test lists
  (emit-identity "@begin(list)@item(a)@end(list)")
  (emit-identity "@begin(list)@item(a)@item(b)@end(list)")
  (emit-identity "@begin(enum)@item(a)@item(b)@end(enum)")
  (emit-identity "@begin(deflist)@term(a)@def(1)@end(deflist)"))

(test images
  (emit-identity "@image[src=a]()"))

(test tables
  (emit-identity "@begin(table)@begin(row)@cell(a)@end(row)@end(table)"))

(test section
  (emit-identity "@begin(section)@title(sec)test@end(section)"))
